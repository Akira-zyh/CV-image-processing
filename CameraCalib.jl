...
cv2 = pyimport("cv2")
# test_img = cv2.imread(".\\CameraCalib\\CameraCalib\\test_image.jpg",0)
# imshow(test_img)
# # 执行非线性标定
# fit = calib_nonlinear(obj_p, img_lis, p0)
...
for i in 0:15
    ...
    imshow(undistorted_test_img)
    # 保存矫正后的图像
    cv2.imwrite("data/part3/undistorted_$i.jpg", undistorted_test_img)

    println("已完成图像畸变矫正，并保存矫正后的图像为undistorted_$i.jpg")
end
